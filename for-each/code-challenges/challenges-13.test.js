...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map( num => `${sum.substring(1,4)}${num.substring(6,9)}${num.substring(10)}`);
};
...
const unenrollBrook = (arr) => {
  // Solution code here...
  return arr.map( course => course.filter(person => !personalbar.includes('Brook')));
};
